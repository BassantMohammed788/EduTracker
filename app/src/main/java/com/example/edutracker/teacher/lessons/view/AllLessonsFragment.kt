...
class AllLessonsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.addLessonFAB.setOnClickListener {
            Navigation.findNavController(requireView()).navigate(R.id.action_allLessonsFragment_to_addLessonFragment)
        }
    ...
    private fun displayGroupDialog(){
        ...
        if (checkConnectivity(requireContext())){
            if (gradeLevel(gradeVar)!= null){
                lifecycleScope.launch {
                    groupsViewModel.getAllGroups(semesterVar!!,MySharedPreferences.getInstance(requireContext()).getTeacherID()!!,gradeLevel(gradeVar)!!)
    ...
    private val lessonDeleteLambda = { lesson: Lesson ->
        ...
        alertDialog.dialogYesBtn.setOnClickListener {
            if (checkConnectivity(requireContext())){
                lifecycleScope.launch {
                    lessonViewModel.deleteLesson(semesterVar!!,teacherIdVar!!, gradeLevel(gradeVar!!)!!,groupIdVar!!,monthVar!!,lesson.id)
                }
            }
